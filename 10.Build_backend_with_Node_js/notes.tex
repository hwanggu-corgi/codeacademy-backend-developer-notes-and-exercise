...
\subsection{Create an HTTP Server}
\begin{itemize}
    \item Node was designed with back end development needs as a top priority
    \item \texttt{http.createServer()} - returns an instance of an http.server
    \begin{itemize}
        \item has a method .listen() which causes the server to “listen” for incoming connections.
        \item a custom callback function is passed (often referred to as the requestListener)
    \end{itemize}
...
\end{lstlisting}
    ...
    \item how \texttt{requestListener} callback function works:
    \begin{itemize}
        \item expects two arguments: a \texttt{request} object and a \texttt{response} object.
        \item Each time a request to the server is made, Node will invoke the provided \texttt{requestListener} callback function
    \end{itemize}
